...
public class Preprocess : MonoBehaviour
{
    ...
    public void ScaleAndCropImage(RawImage webCamTexture, int desiredSize, UnityAction<byte[]> callback)
    {
        ...
        scale.x = (float)webCamTexture.texture.height / (float)webCamTexture.texture.width;
        offset.x = (1 - scale.x) / 2f;
        Graphics.Blit(webCamTexture.texture, renderTexture, scale, offset);
        AsyncGPUReadback.Request(renderTexture, 0, TextureFormat.RGB24, OnCompleteReadback);
    }
